...
import geotrellis.spark.{Metadata, TileLayerMetadata}
...
object RDDPolygonalSummary {
  ...
  trait RDDPolygonalSummaryMethods[R, T <: CellGrid[Int]]
    ...
    def polygonalSummary[R](geometries: Seq[Geometry],
                            visitor: GridVisitor[Raster[T], R],
                            options: Rasterizer.Options)(
        implicit semigroupR: Semigroup[R], tagR: ClassTag[R], sc: SparkContext): RDD[Feature[Geometry, PolygonalSummaryResult[R]]] = {
      self.polygonalSummary(sc.parallelize(geometries), visitor, options)
    }

    def polygonalSummary[R](geometries: Seq[Geometry],
                            visitor: GridVisitor[Raster[T], R])(
        implicit semigroupR: Semigroup[R], tagR: ClassTag[R], sc: SparkContext): RDD[Feature[Geometry, PolygonalSummaryResult[R]]] = {
      self.polygonalSummary(sc.parallelize(geometries),
                            visitor,
    ...
    def polygonalSummaryValue[R](geometry: Geometry,
                                 visitor: GridVisitor[Raster[T], R],
                                 options: Rasterizer.Options)(
        implicit semigroupR: Semigroup[R], tagR: ClassTag[R], sc: SparkContext): PolygonalSummaryResult[R] = {
      self
        .polygonalSummary(sc.parallelize(List(geometry)), visitor, options)
        .map { _.data }
        .reduce { _.combine(_) }
    }
